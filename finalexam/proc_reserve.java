...
public class proc_reserve extends ViewableAtomic
{
	...
	public proc_reserve(String name, double Processing_time)
	{
		super(name);
	...
	public message out()
	{
		...
		if(job_ex._option < 6) { //option:1~5인 경우만 수용(meal에게 전달)
			if (phaseIs("busy"))
			{
				m.add(makeContent("out", new job("age:" + job_ex._age, job_ex._option, job_ex._plane, job_ex._age, job_ex._meal)));
				System.out.println("\n PROC RESERVE");
